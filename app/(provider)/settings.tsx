    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}> 
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 116 }}> 
        <View style={styles.header}> 
          ...
          <Text style={[styles.headerSubtitle, { color: colors.textMuted }]}>نسخة مقدم الخدمة</Text>
        </View>
        ...
        <MenuRow icon="eye-outline" label="معاينة الملف العام" subtitle="افتح صفحتك كما يراها العملاء" color={colors.gold} onPress={() => router.push('/(provider)/profile' as never)} colors={colors} />
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 54, height: 54, borderRadius: 17, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 22, fontFamily: 'Cairo-Black' },
  name: { fontSize: 16, fontFamily: 'Cairo-Bold', textAlign: 'right' },
  email: { marginTop: 2, fontSize: 12, fontFamily: 'Cairo-Regular', textAlign: 'right' },
  sectionTitle: { paddingHorizontal: 24, marginBottom: 12, fontSize: 12, fontFamily: 'Cairo-Bold', textAlign: 'right', writingDirection: 'rtl' },
  menuRow: { marginHorizontal: 16, marginBottom: 12, paddingVertical: 12, paddingHorizontal: 12, borderRadius: 22, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
